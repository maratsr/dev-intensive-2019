...
class GroupActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean { // Обработчик нажатия перехода на пред экран (home)
    ...
    private fun initToolbar() {
        ...
        supportActionBar?.setDisplayHomeAsUpEnabled(true) // Для появления стрелки перехода на пред экран
    }
    ...
    private fun initViews() {
        ...
        fab.setOnClickListener {
            ...
            finish()
            overridePendingTransition(R.anim.idle, R.anim.bottom_down) // завершение активити с анимацией (из какой и куда
        }
    }
    ...
    private fun updateChips(listUsers: List<UserItem>) {
        ...
        for(k in listUsers) {
            if (k.isSelected)
                Log.d("M_GroupActivity", "User= ${k.id} ${k.fullName}")
        }
        ...
        users.forEach { v-> Log.d("M_GroupActivity", "ChipGroup Before = ${v.key} ${v.value.fullName}")}
        ...
        users.forEach { v-> Log.d("M_GroupActivity", "ChipGroup After = ${v.key} ${v.value.fullName}")}
